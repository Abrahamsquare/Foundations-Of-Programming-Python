...
while(True):
    ...
    # Choice 2 - Add a new item to the list/Table
        ...
        task_new = input("What is the name of the task?: ")
        priority_new = input("Enter the new task's priority from low to high: ") 
    ...
    # Choice 4 - Save tasks to the ToDo.txt file
        ...
        if user_save.lower() == "yes":
            with open(infile,"w") as op:
                for key,value in task_dict.items():
                    outline = "{},{}\n".format(key,value)
                    op.write(outline)
        
        # Chocie 5- end the program1
